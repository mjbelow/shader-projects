...
//-----------------------------------------------------
#define F +texture2D(iChannel1,.3+p.xz*s/3e3)/(s+=s) 
...
//  if (!MysteryMountains(gl_FragColor, gl_FragCoord.xy))
//  {
  float bg = background(uv);
  gl_FragColor = vec4(bg*0.9, bg, bg*1.1, 1.0);
//  }
  float snowOut = snowing(uv);
