...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    vec4 bottom_layer = vec4(1, 0, 0, 1);
    ...
    top_mix = mix(1., top_mix, S(gray_dark_1 - EPSILON, max(gray_dark_1,gray_dark_2), top_gray));
    top_mix = mix(top_mix, 1., S(min(gray_light_1,gray_light_2) - EPSILON, gray_light_1, top_gray));
    top_mix = mix(1., top_mix, S(red_dark_1 - EPSILON, max(red_dark_1,red_dark_2), top_red));
    top_mix = mix(top_mix, 1., S(min(red_light_1,red_light_2) - EPSILON, red_light_1, top_red));
    top_mix = mix(1., top_mix, S(green_dark_1 - EPSILON, max(green_dark_1,green_dark_2), top_green));
    top_mix = mix(top_mix, 1., S(min(green_light_1,green_light_2) - EPSILON, green_light_1, top_green));
    top_mix = mix(1., top_mix, S(blue_dark_1 - EPSILON, max(blue_dark_1,blue_dark_2), top_blue));
    top_mix = mix(top_mix, 1., S(min(blue_light_1,blue_light_2) - EPSILON, blue_light_1, top_blue));

    fragColor = mix(top_layer, bottom_layer, top_mix);
